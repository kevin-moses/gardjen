...
export class AudioDisplay {
  ...
  handleResize() {
    if (this.historyCanvas) {
      ...
      if (container) {
        ...
        this.drawHistory(); // Redraw on resize
      }
    }
  }
   ...
   */
  drawHistory() {
    if (!this.historyCanvas || !this.historyContext) return;
    ...
    if (this.historyData.rms.length === 0) return;
    
    // Draw each metric line
    this.drawMetricLine('rms', this.colors.rms);
    this.drawMetricLine('energy', this.colors.energy);
    this.drawMetricLine('spectralFlux', this.colors.spectralFlux);
    this.drawMetricLine('spectralCentroid', this.colors.spectralCentroid);
    this.drawMetricLine('zcr', this.colors.zcr);
  }
